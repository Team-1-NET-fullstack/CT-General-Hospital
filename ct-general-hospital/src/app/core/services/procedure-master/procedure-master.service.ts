...
export class ProcedureMasterService {
  ...
  getAllProcedurebyDesc(desc:string) {
    return this.masterClient.get<ProcedureMaster[]>('http://localhost:9001/api/ProcedureMasters/GetProcedureByDescription?desc='+desc);
  }
  createProcedure(procedure: ProcedureMaster) {
    
    this.masterClient
      .post('http://localhost:9001/api/ProcedureMasters/CreateNewProcedure', procedure)
      .subscribe((res) => {
        console.log(res);
